...
class PelatihanAk1Model extends Model
{
    ...
    return PelatihanAk1Model::insertGetId($data);
    }
    ...
    public function deletePelatihan($rand_ak1_pelatihan)
    {
        ...
        return PelatihanAk1Model::where('rand_ak1_pelatihan', '=', $rand_ak1_pelatihan)->delete();
}
    ...
    public function updatePelatihan($data,$rand_ak1_pelatihan)
    {
        ...
        PelatihanAk1Model::where('rand_ak1_pelatihan',$rand_ak1_pelatihan)->update($data);
    }
    public function allData($rand_ak1)
    {
        // if(DB::table('tbl_ak1_pelatihan')->where('rand_ak1',$rand_ak1)->where('NA',1)->get()->isNotEmpty())
        if(PelatihanAk1Model::where('rand_ak1',$rand_ak1)->where('NA',1)->get()->isNotEmpty())
        {
            // return DB::table('tbl_ak1_pelatihan')
            // ->where('rand_ak1',$rand_ak1)
            // ->where('NA',1)
            // ->get();
            return PelatihanAk1Model::where('rand_ak1',$rand_ak1)->where('NA',1)->get();
        }
        else {
            return NULL;
        }
    ...
    public function pdfData($rand_ak1)
    {
        ...
        if(PelatihanAk1Model::where('rand_ak1',$rand_ak1)->where('NA',1)->get()->isNotEmpty())
        {
            ...
            // ->where('dipakai',1)
            // ->get();
            return PelatihanAk1Model::where('rand_ak1',$rand_ak1)->where('NA',1)->where('dipakai',1)->get();
        }
        else {
            return NULL;
        }
